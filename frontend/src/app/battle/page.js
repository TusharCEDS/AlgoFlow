    ...
    return (
      <main className="min-h-screen bg-gray-950 text-white flex items-center justify-center px-4">
        <div className="max-w-md w-full">
          <h1 className="text-3xl font-bold mb-2 text-center">⚔️ Battle Mode</h1>
          <p className="text-gray-400 text-center mb-8">Challenge a friend to a real-time coding battle</p>
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-xl p-6 mb-4">
            ...
            <p className="text-gray-400 text-sm mb-4">Start a new battle and share the room code with your friend</p>
            <button
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
            ...
            <p className="text-gray-400 text-sm mb-4">Enter a room code shared by your friend</p>
            <input
              type="text"
              value={joinRoomId}
    ...
    return (
      <main className="min-h-screen bg-gray-950 text-white flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-3xl font-bold mb-4">⚔️ Waiting for opponent...</h1>
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-xl px-12 py-6 inline-block mb-6">
            <span className="text-4xl font-mono font-bold text-blue-400 tracking-widest">{roomId}</span>
          </div>
          <p className="text-gray-500 text-sm">Battle will start automatically when they join</p>
        </div>
      </main>
    )
  }
      ...
      <main className="min-h-screen bg-gray-950 text-white flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-400 mb-8">
            {isWinner ? 'Congratulations! You solved it first.' : 'Better luck next time!'}
          </p>
          <button
            onClick={() => {
              setScreen('lobby')
    ...
    <main className="min-h-screen bg-gray-950 text-white">
      ...
      <div className="flex items-center justify-between px-8 py-3 border-b border-gray-800 bg-gray-900">
        <div className="flex items-center gap-4">
          <span className="text-sm text-gray-400">Room: <span className="text-white font-mono font-bold">{roomId}</span></span>
          <span className="text-sm text-gray-400">
            {players.map((p, i) => (
              <span key={i}>
                <span className={p === user?.username ? 'text-blue-400' : 'text-red-400'}>{p}</span>
                {i < players.length - 1 && <span className="text-gray-600 mx-2">vs</span>}
              </span>
            ))}
          </span>
        </div>
